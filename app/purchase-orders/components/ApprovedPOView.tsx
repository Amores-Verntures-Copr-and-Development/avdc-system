...
const ApprovedPOView: React.FC<ApprovedPOViewProps> = ({ data, onSendPO }) => {
  const [expandedSupplier, setExpandedSupplier] = useState<number | null>(null);
  const handleSendBySupplier = async (data: PurchaseOrderItems[]) => {
    ...
    <div className="bg-white">
      ...
      <div className="flex flex-col p-4">
        ...
        <div className="space-y-4">
          {data.map((data) => {
            const isSupplierItemsSent = data.items?.every(
              (items) => items.isSent === 1
            );
            return (
              <div
                          ...
                          {isSupplierItemsSent ? (
                            ...
                            <div>
                              ...
                              <Button
                                isRounded={false}
                                size="xs"
                                onClick={() => handleSendBySupplier(data.items)}
                                color="nocolor"
                                label="Send"
                                icon={
                                  <Send size={15} className="text-primary-1 " />
                                }
                                className="font-semibold"
                              />
                            </div>
